...
impl QLinear {
    fn from_vb(prefix: String, vb: &QVarBuilder) -> Result<Self> {
        let weight_q = vb.get_no_shape(&format!("{prefix}.weight"))?;
        let weight = QMatMul::from_arc(weight_q)?;
    ...
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let mut y = self.weight.forward(xs)?;
...
impl Attention {
    ...
    fn forward(
        ...
        attn_output
            .transpose(1, 2)?
            .reshape((b_sz, q_len, self.num_heads * self.head_dim))
            .and_then(|x| self.o_proj.forward(&x))
    }
...
impl Layer {
    ...
    fn forward(
        ...
        let xs = (xs + residual)?;
        let residual = &xs;
        let xs = self.mlp_norm.forward(&xs)?;
...
#[derive(Debug, Clone)]
pub struct QuantizedCsmModel {
    pub backbone: LlamaModel,
    pub decoder: LlamaModel,
    codebook0_head: QLinear,
...
impl Csm for QuantizedCsmModel {
    fn clear_kv_cache(&mut self) {
    ...
    fn generate_frame(
        ...
        for i in 1..self.config.audio_num_codebooks {
            let proj_h = self.projection.forward(&curr_h)?;
